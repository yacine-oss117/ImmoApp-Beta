...
class ClientDict(TypedDict):
    """A dictionary representation of a Client for database interchange."""
...
class Client:
    """
    A client looking to buy or rent property.
    ...
    tags: str = ""
    is_vip: bool = False
    status: str = "active"  # active, archived_rented, archived_sold
    deleted_at: str = ""
    created_at: str = ""
    created_loc: str = ""
    updated_at: str = ""
    ...
    @classmethod
    def from_row(cls, row: Mapping[str, object]) -> Client:
        """Create a Client from a database row with transparent ALE decryption."""
        ...
        if family_name_enc and enc is not None:
            try:
                ...
                logger.debug(
                    "Failed to decrypt client family_name_enc; falling back to plaintext",
                    exc_info=True,
                )
        ...
        if phone_enc and enc is not None:
            try:
                ...
                logger.debug(
                    "Failed to decrypt client phone_enc; falling back to plaintext",
                    exc_info=True,
                )
        ...
        if remarks_enc and enc is not None:
            try:
                ...
                logger.debug(
                    "Failed to decrypt client remarks_enc; falling back to plaintext",
                    exc_info=True,
                )
        ...
        return cls(
            ...
            status=as_str(row_value(row, "status"), default="active"),

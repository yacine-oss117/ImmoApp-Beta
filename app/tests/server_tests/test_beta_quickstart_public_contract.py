...
def test_windows_powershell_web_requests_use_basic_parsing() -> None:
    ...
    assert (
        "Invoke-WebRequest -Method Get -Uri $healthUri -Headers $probeHeaders -TimeoutSec 3 -UseBasicParsing"
        in stack
    )
    assert (
        "Invoke-WebRequest -Method Get -Uri \"$($Addr.TrimEnd('/'))/v1/sys/health\" -TimeoutSec 3 -UseBasicParsing"
        in common
    )
